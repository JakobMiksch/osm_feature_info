...
RETURNS TABLE( osm_type char, osm_id bigint, tags jsonb, geom geometry
)
AS $$
BEGIN
	RETURN QUERY
			SELECT  gn.osm_type, gn.osm_id, r.tags, gn.geom   FROM geom_nodes AS gn
			...
			SELECT  gw.osm_type, gw.osm_id, r.tags, gw.geom  FROM geom_ways AS gw
			...
			SELECT  gr.osm_type, gr.osm_id, r.tags, gr.geom  FROM geom_rels AS gr
...
-- example request
-- SELECT * FROM postgisftw.osm_feature_info(49.42219177, 8.67988265, 10);
